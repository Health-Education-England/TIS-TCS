...
@Component({
    selector: 'jhi-programme-tis-programmes',
    templateUrl: './programme-tis-programmes.component.html'
})
export class ProgrammeTisProgrammesComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/programme-tis-programmes'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/programme-tis-programmes', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId (index: number, item: ProgrammeTisProgrammes) {
        return item.id;
    }
