...
public class DocumentServiceImpl implements DocumentService {
  ...
  @Override
  public Page<DocumentDTO> findAll(final Long personId, final String query,
      final List<ColumnFilter> columnFilters, final List<String> tagNames,
  ...
  @Override
  public void download(final DocumentDTO document, final OutputStream outputStream)
      throws IOException {
    LOG.debug("Received request to download '{}' with ID '{}'",
        DocumentDTO.class.getSimpleName(),
        document.getId());

    final Optional<Document> documentOptional = documentRepository
        .findOneByPersonIdAndIdAndStatus(document.getPersonId(), document.getId(), Status.CURRENT);

    if (!documentOptional.isPresent()) {
      LOG.warn("Document with id '{}' and person id '{}' does not exist", document.getPersonId(),
          document.getId());
      ...
      try {
        ...
        try {
          fileStorageRepository
              .deleteFile(azureProperties.getContainerName(), getAzureFilePath(document));
  ...
  public void save(final Collection<DocumentDTO> documentDTOs) throws IOException {
    ...
    for (final DocumentDTO documentParam : documentDTOs) {
      ...
      final Optional<Document> existingDocumentOptional = documentRepository
          .findById(documentParam.getId());
  ...
  private void updateDocumentMetadata(final Document existingDocument,
      final DocumentDTO documentParamDTO) {
    ...
    final Set<Tag> databaseTags = Optional.ofNullable(documentParam.getTags())
        ...
        .stream()
        .map(tag -> Optional.ofNullable(tagRepository.findByName(tag.getName())).orElse(tag))
  ...
  public Optional<DocumentDTO> delete(final Long personId, final Long documentId) {
    final Optional<Document> documentOptional = documentRepository
        .findOneByPersonIdAndIdAndStatus(personId, documentId, Status.CURRENT);
  ...
  private String saveFile(final Document document) throws
    ...
    return fileStorageRepository.store(null, azureProperties.getContainerName(),
        Lists.newArrayList(getFileAsMultipart(document)));
  }
  ...
  private Specifications<Document> addSearchQueryToSpec(Specifications<Document> fullSpec,
    ...
    querySpecs
        .add((root, criteriaQuery, sb) -> sb.like(root.get("fileExtension"), "%" + query + "%"));
    querySpecs
