...
public class FundingComponentsResource {
     ...
     * @return the ResponseEntity with status 200 (Created) and with body the new fundingComponentsDTOS, or with status 400 (Bad Request) if the FundingComponents has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/bulk-funding-components")
    @Timed
    @PreAuthorize("hasAuthority('tcs:add:modify:entities')")
    public ResponseEntity<List<FundingComponentsDTO>> bulkCreateFundingComponents(@Valid @RequestBody List<FundingComponentsDTO> fundingComponentsDTOS) throws URISyntaxException {
        ...
        if (!Collections.isEmpty(fundingComponentsDTOS)) {
            ...
            if (!Collections.isEmpty(entityIds)) {
                return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(StringUtils.join(entityIds, ","), "ids.exist", "A new FundingComponents cannot already have an ID")).body(null);
            }
        }
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated fundingComponentsDTOS,
     * or with status 400 (Bad Request) if the fundingComponentsDTOS is not valid,
     * or with status 500 (Internal Server Error) if the fundingComponentsDTOS couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/bulk-funding-components")
    @Timed
    @PreAuthorize("hasAuthority('tcs:add:modify:entities')")
    public ResponseEntity<List<FundingComponentsDTO>> bulkUpdateFundingComponents(@Valid @RequestBody List<FundingComponentsDTO> fundingComponentsDTOS) throws URISyntaxException {
        ...
        if (Collections.isEmpty(fundingComponentsDTOS)) {
            ...
            List<FundingComponentsDTO> entitiesWithNoId = fundingComponentsDTOS.stream().filter(f -> f.getId() == null).collect(Collectors.toList());
