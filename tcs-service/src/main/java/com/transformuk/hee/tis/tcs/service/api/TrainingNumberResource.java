...
public class TrainingNumberResource {
  ...
  public TrainingNumberResource(TrainingNumberService trainingNumberService,
      TrainingNumberValidator trainingNumberValidator) {
   ...
   * @return the ResponseEntity with status 201 (Created) and with body the new trainingNumberDTO,
   * or with status 400 (Bad Request) if the trainingNumber has already an ID
   * @throws URISyntaxException if the Location URI syntax is incorrect
   */
  @PostMapping("/training-numbers")
  @PreAuthorize("hasAuthority('tcs:add:modify:entities')")
  public ResponseEntity<TrainingNumberDTO> createTrainingNumber(
      @RequestBody @Validated(Create.class) TrainingNumberDTO trainingNumberDTO)
      ...
      if (trainingNumberDTO.getId() != null) {
        return ResponseEntity.badRequest().headers(HeaderUtil
            .createFailureAlert(ENTITY_NAME, "idexists",
                "A new trainingNumber cannot already have an ID")).body(null);
      }
   ...
   * @return the ResponseEntity with status 200 (OK) and with body the updated trainingNumberDTO, or
   * with status 400 (Bad Request) if the trainingNumberDTO is not valid, or with status 500
   * (Internal Server Error) if the trainingNumberDTO couldnt be updated
   * @throws URISyntaxException if the Location URI syntax is incorrect
   */
  @PutMapping("/training-numbers")
  @PreAuthorize("hasAuthority('tcs:add:modify:entities')")
  public ResponseEntity<TrainingNumberDTO> updateTrainingNumber(
      @RequestBody @Validated(Update.class) TrainingNumberDTO trainingNumberDTO)
      throws URISyntaxException, MethodArgumentNotValidException {
    ...
    try {
      ...
      return ResponseEntity.ok()
          .headers(
              HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, trainingNumberDTO.getId().toString()))
          .body(result);
    } catch (DataIntegrityViolationException e) {
      log.error(e.getMessage(), e);
  ...
  public ResponseEntity<List<TrainingNumberDTO>> getAllTrainingNumbers(Pageable pageable) {
    ...
    HttpHeaders headers = PaginationUtil
        .generatePaginationHttpHeaders(trainingNumberDTOPage, "/api/training-numbers");
   ...
   * @return the ResponseEntity with status 200 (OK) and with body the trainingNumberDTO, or with
   ...
   * @return the ResponseEntity with status 200 (Created) and with body the new trainingNumberDTOS,
   * or with status 400 (Bad Request) if the Training Number has already an ID
   */
  @PostMapping("/bulk-training-numbers")
  @PreAuthorize("hasAuthority('tcs:add:modify:entities')")
  public ResponseEntity<List<TrainingNumberDTO>> bulkCreateTrainingNumbers(
      @Valid @RequestBody List<TrainingNumberDTO> trainingNumberDTOS) {
      ...
      if (!Collections.isEmpty(entityIds)) {
        return ResponseEntity.badRequest().headers(HeaderUtil
            .createFailureAlert(StringUtils.join(entityIds, ","), "ids.exist",
                "A new Training Numbers cannot already have an ID")).body(null);
      }
    }
   ...
   * or with status 400 (Bad Request) if the trainingNumberDTOS is not valid, or with status 500
   * (Internal Server Error) if the trainingNumberDTOS couldnt be updated
   */
  @PutMapping("/bulk-training-numbers")
  @PreAuthorize("hasAuthority('tcs:add:modify:entities')")
  public ResponseEntity<List<TrainingNumberDTO>> bulkUpdateTrainingNumbers(
      @Valid @RequestBody List<TrainingNumberDTO> trainingNumberDTOS) {
    ...
    if (Collections.isEmpty(trainingNumberDTOS)) {
      ...
      List<TrainingNumberDTO> entitiesWithNoId = trainingNumberDTOS.stream()
          .filter(tn -> tn.getId() == null).collect(Collectors.toList());
