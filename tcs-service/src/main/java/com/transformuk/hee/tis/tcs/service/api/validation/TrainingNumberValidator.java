...
/**
 * Holds more complex custom validation for a {@link TrainingNumberDTO} that cannot be easily done
 * via annotations
 */
@Component
public class TrainingNumberValidator {
  ...
  /**
   * Custom validation on the trainingNumber DTO, this is meant to supplement the annotation based
   * validation already in place. It checks if the number field in trainingNumber is unique.
   *
  ...
  public void validate(TrainingNumberDTO trainingNumberDTO) throws MethodArgumentNotValidException {
    ...
    if (!fieldErrors.isEmpty()) {
      BeanPropertyBindingResult bindingResult = new BeanPropertyBindingResult(trainingNumberDTO,
          "TrainingNumberDTO");
  ...
  private List<FieldError> checkNumber(TrainingNumberDTO trainingNumberDTO) {
    ...
    if (trainingNumberDTO.getId() != null) {
      List<TrainingNumber> trainingNumberList = trainingNumberRepository
          .findByNumber(trainingNumberDTO.getNumber());
      if (trainingNumberList.size() > 1) {
        fieldErrors.add(new FieldError("TrainingNumberDTO", "number",
            String.format(
                "Number %s is not unique, there are currently %d training numbers with this number: %s",
        ...
        if (!trainingNumberDTO.getId().equals(trainingNumberList.get(0).getId())) {
          fieldErrors.add(new FieldError("TrainingNumberDTO", "number",
              String.format(
                  "Number %s is not unique, there is currently one training number with this number: %s",
      ...
      if (!trainingNumberList.isEmpty()) {
        fieldErrors.add(new FieldError("TrainingNumberDTO", "number",
            String.format(
                "Number %s is not unique, there is currently one programme with this number: %s",
                trainingNumberDTO.getNumber(), trainingNumberList.get(0))));
      }
    }
    return fieldErrors;
