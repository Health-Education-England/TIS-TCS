...
public class PersonBasicDetailsRepositoryAccessor {
  ...
  @Autowired
  public PersonBasicDetailsRepositoryAccessor(
  ...
  public void doWithPersonBasicDetailsSet(Set<Long> personIds,
      Consumer<Map<Long, PersonBasicDetails>> consumer) {
    if (CollectionUtils.isNotEmpty(personIds)) {
      List<PersonBasicDetails> details = personBasicDetailsRepository.findAllById(personIds);
      if (CollectionUtils.isNotEmpty(details)) {
        Map<Long, PersonBasicDetails> detailsMap = details.stream()
            .collect(Collectors.toMap(PersonBasicDetails::getId, d -> d));
