...
public class PlacementValidator {
    ...
    public void validate(final PlacementDetailsDTO placementDetailsDTO) throws ValidationException {
        ...
        if (!fieldErrors.isEmpty()) {
            final BeanPropertyBindingResult bindingResult = new BeanPropertyBindingResult(placementDetailsDTO, PLACEMENT_DTO_NAME);
    ...
    private List<FieldError> checkSite(final PlacementDetailsDTO placementDetailsDTO) {
        ...
        if (placementDetailsDTO.getSiteId() == null) {
            ...
            final Map<Long, Boolean> siteIdsExistsMap = referenceService.siteIdExists(newArrayList(placementDetailsDTO.getSiteId()));
    ...
    private List<FieldError> checkGrade(final PlacementDetailsDTO placementDetailsDTO) {
        ...
        if (placementDetailsDTO.getGradeId() == null) {
            ...
            final Map<Long, Boolean> gradeIdsExistsMap = referenceService.gradeIdsExists(newArrayList(placementDetailsDTO.getGradeId()));
    ...
    private void notExistsStringFieldErrors(final List<FieldError> fieldErrors, final Map<String, Boolean> gradeIdsExistsMap,
                                            final String field, final String entityName) {
        gradeIdsExistsMap.forEach((k, v) -> {
            if (!v) {
                fieldErrors.add(new FieldError(PLACEMENT_DTO_NAME, field,
    ...
    private void notExistsFieldForLongIdsErrors(final List<FieldError> fieldErrors, final Map<Long, Boolean> gradeIdsExistsMap,
                                                final String field, final String entityName) {
        gradeIdsExistsMap.forEach((k, v) -> {
            if (!v) {
                fieldErrors.add(new FieldError(PLACEMENT_DTO_NAME, field,
