...
/**
 * Holds more complex custom validation for a {@link Person} that
...
public class PersonValidator {
  ...
  /**
   * Custom validation on the personDTO DTO, this is meant to supplement the annotation based validation
   * already in place. It checks that the public health number unique check if it is entered.
   *
  ...
  public void validate(PersonDTO personDTO) throws MethodArgumentNotValidException {
    ...
    if (!fieldErrors.isEmpty()) {
      BeanPropertyBindingResult bindingResult = new BeanPropertyBindingResult(personDTO, PERSON_DTO_NAME);
      ...
      if (StringUtils.isNotEmpty(publicHealthNumber)) {
        ...
        if (existingPersons.size() > 1) {
          fieldErrors.add(new FieldError(PERSON_DTO_NAME, "publicHealthNumber",
                  String.format("publicHealthNumber %s is not unique, there are currently %d persons with this number: %s",
          ...
          if (!personDTO.getId().equals(existingPersons.get(0).getId())) {
            fieldErrors.add(new FieldError(PERSON_DTO_NAME, "publicHealthNumber",
                    String.format("publicHealthNumber %s is not unique, there is currently one person with this number: %s",
