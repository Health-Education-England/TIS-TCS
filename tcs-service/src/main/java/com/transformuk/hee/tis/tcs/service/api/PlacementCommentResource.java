...
public class PlacementCommentResource {
  ...
  public ResponseEntity<PlacementCommentDTO> getPlacementComment(
  ...
  @PreAuthorize("hasAuthority('tcs:view:entities')")
  //TODO verify if this permission is right/present
  public ResponseEntity<PlacementCommentDTO> getPlacementCommentByPlacementId(
      @PathVariable("placementId") final Long placementId) {
   ...
   * @return the ResponseEntity with status 201 (Created) and with body the new placementCommentDTO,
   * or with status 400 (Bad Request) if the placementCommentDTO has already an ID
   * @throws URISyntaxException if the Location URI syntax is incorrect
   */
  @PostMapping(PLACEMENT_COMMENT)
  @PreAuthorize("hasAuthority('tcs:add:modify:entities')")
  //TODO setup a perm/authority with fewer perms
  public ResponseEntity<PlacementCommentDTO> createPlacementComment(
      @RequestBody @Validated(Create.class) final PlacementCommentDTO placementCommentDTO)
    ...
    if (placementCommentDTO.getId() != null) {
      return ResponseEntity.badRequest().headers(
          HeaderUtil.createFailureAlert(ENTITY_NAME, IDEXISTS, A_NEW_PLACEMENT_CANNOT_HAVE_AN_ID))
          .body(null);
    }
  ...
  @PreAuthorize("hasAuthority('tcs:add:modify:entities')")
  //TODO setup a perm/authority with fewer perms
  public ResponseEntity<PlacementCommentDTO> updatePlacementComment(
      @RequestBody @Validated(Update.class) final PlacementCommentDTO placementCommentDTO)
      throws URISyntaxException, ValidationException {
