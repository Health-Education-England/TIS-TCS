...
/**
 * Holds more complex custom validation for a {@link ContactDetails} that
...
public class ContactDetailsValidator {
  ...
  public void validate(ContactDetailsDTO contactDetailsDTO) throws MethodArgumentNotValidException {
    ...
    if (!fieldErrors.isEmpty()) {
      BeanPropertyBindingResult bindingResult = new BeanPropertyBindingResult(contactDetailsDTO, "ContactDetailsDTO");
  ...
  private List<FieldError> checkTitle(ContactDetailsDTO contactDetailsDTO) {
    ...
    if (StringUtils.isNotEmpty(contactDetailsDTO.getTitle())) {
      Boolean isExists = referenceService.isValueExists(TitleDTO.class, contactDetailsDTO.getTitle());
      if (!isExists) {
