...
public interface SpecialtyRepository extends JpaRepository<Specialty, Long>,
  ...
  Page<Specialty> findBySpecialtyGroupIdIs(Long groupId, Pageable pageable);

  Set<Specialty> findBySpecialtyGroupIdIs(Long groupId);
  ...
  /**
   * Find a page of specialties related to a Programme via the Curricula where the specialty name is
   * like the provided name
   *
   * @param programmeId The programme Id of the programme
  ...
  @Query("SELECT DISTINCT sp " +
      ...
      "AND sp.status = :status ")
  Page<Specialty> findSpecialtiesByProgrammeId(@Param("programmeId") Long programmeId,
      @Param("status") Status status, Pageable pageable);

  /**
   * Get a Page of Specialties linked to a Post whereby the Post is linked to a Programme Id and the
   * Specialty name is like the provided query
   *
   * @param programmeId The Programme Id
  ...
  Page<Specialty> findSpecialtiesByProgrammeIdAndName(@Param("programmeId") Long programmeId,
  ...
  @Query("SELECT DISTINCT sp " +
      ...
      "AND t.id = :personId " +
      "AND sp.status = :status")
  Set<Specialty> findDistinctByProgrammeIdAndPersonIdAndStatus(
      @Param("programmeId") Long programmeId, @Param("personId") Long personId,
