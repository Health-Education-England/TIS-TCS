...
from (SELECT p.`id`,
    ...
    LEFT JOIN (
        ...
        FROM `Placement` pl
        LEFT JOIN `ContactDetails` c on pl.`traineeId` = c.`id` WHERE curdate() BETWEEN pl.`dateFrom` AND pl.`dateTo`
        GROUP BY pl.`postId`
...
primarySiteId,nationalPostNumber,fundingStatus,owner,surnames,forenames,intrepidId
ORDERBYCLAUSE
LIMITCLAUSE
;
