...
public class SpecialtyRepositoryTest {
  ...
  public void findSpecialtyDistinctByCurriculaProgrammesIdShouldGetSpecialtyForProgrammeId() {
    ...
    for (Specialty specialty : result.getContent()) {
      Assert.assertTrue("SPE-1".equals(specialty.getSpecialtyCode()) || "SPE-2"
          .equals(specialty.getSpecialtyCode()));
    }
  }
  ...
  public void findSpecialtyByCurriculaProgrammesIdAndNameIsLikeIgnoreCaseShouldReturnSpecialtiesWithNameLikeAndLinkedToProgrammeId() {
    ...
    Assert.assertFalse(result.hasNext());
    for (Specialty specialty : result.getContent()) {
      Assert.assertTrue(("Metabolic Medicine".equals(specialty.getName()) && "SPE-5"
          .equals(specialty.getSpecialtyCode()) ||
          ("Oral Medicine".equals(specialty.getName()) && "SPE-6"
              .equals(specialty.getSpecialtyCode()))));
    }
  }
  ...
  public void findDistinctByProgrammeIdAndPersonId() {
    ...
    Set<Specialty> results = specialtyRepository
        .findDistinctByProgrammeIdAndPersonIdAndStatus(programmeId, personId, Status.CURRENT);

    Assert.assertEquals(2, results.size());
    for (Specialty result : results) {
      Assert.assertTrue(expectedSpecialtyId1.equals(result.getId()) || expectedSpecialtyId2
          .equals(result.getId()));
      Assert.assertTrue(
          specialtyName1.equals(result.getName()) || specialtyName2.equals(result.getName()));
    }
  }
}
